...
@router.post("")
def create_interaction(interaction_request: InteractionRequest, db: Session = Depends(get_db)):
    interaction_service = InteractionService(InteractionRepository(db))
    interaction = interaction_service.create_interaction(interaction_request.input_text, interaction_request.output_text)

    metric_service = MetricService(MetricRepository(db))
    metric = metric_service.calculate_and_store_metric(interaction.id, interaction_request.input_text, interaction_request.output_text)

    alert_service = AlertService(AlertRepository(db))
    all_metrics = [metric.input_metric, metric.output_metric]
    alerts = alert_service.check_and_create_alert(interaction.id, "input", metric.input_metric, all_metrics)
    alerts += alert_service.check_and_create_alert(interaction.id, "output", metric.output_metric, all_metrics)
...
def get_interactions(db: Session = Depends(get_db)):
    ...
    return create_response(success=True, data=interactions, message="Interactions fetched successfully")
...
async def upload_csv(background_tasks: BackgroundTasks, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    return create_response(success=True, message="CSV file received. Processing in the background.")
