    ...
    def process_csv(self, file_path: str):
        with open(file_path, newline='') as csvfile:
            ...
            for row in reader:
                ...
                interaction = self.interaction_service.create_interaction(input_text, output_text)
                metric = self.metric_service.calculate_and_store_metric(interaction.id, input_text, output_text)
                all_metrics = [metric.input_metric, metric.output_metric]
                self.alert_service.check_and_create_alert(interaction.id, "input", metric.input_metric, all_metrics)
                self.alert_service.check_and_create_alert(interaction.id, "output", metric.output_metric, all_metrics)
